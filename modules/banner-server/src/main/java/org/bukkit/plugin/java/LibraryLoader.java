...
class LibraryLoader {
    ...
    public ClassLoader createLoader(@NotNull PluginDescriptionFile desc) {
        ...
        BannerMCStart.LOGGER.info("[{}] Loading {} libraries... please wait", desc.getName(), desc.getLibraries().size());
        ...
        BannerMCStart.LOGGER.info("[{}] Loading {} extra libraries... please wait", desc.getName(), newDependencies.size() - desc.getLibraries().size());
        ...
        for (File file : libraries) {
            try {
                ...
                BannerMCStart.LOGGER.info("[{}] Loaded libraries {}", desc.getName(), file);
    ...
    public boolean has(Dependency dependency) {
        ...
        if (file.exists()) {
            if (!dependencyIgnoreVersion.contains(dependency.toIgnoreVersion())) {
                libraries.add(file);
                dependencyIgnoreVersion.add(dependency.toIgnoreVersion());
                BannerMCStart.LOGGER.info("[{}] Found libraries {}", dependency.name, file);
                return true;
            }
        }
        return false;
